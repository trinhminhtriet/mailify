...
impl JsonPayload {
    fn into_request(self, name: String) -> mailify_engine::Request {
        mailify_engine::Request {
            name,
            from: self.from,
            to: self.to.into_vec(),
            cc: self.cc.into_vec(),
...
pub(crate) async fn handler(
    ...
    Extension(engine): Extension<mailify_engine::Engine>,
...
mod tests {
    ...
    async fn success() {
        ...
        let smtp_pool = crate::service::smtp::Configuration::insecure(smtp_port)
            .build()
            .unwrap();
        let engine = mailify_engine::Config::default().into();
